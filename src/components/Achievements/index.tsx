  ...
  return (
    <Container>
      ...
      <AchievementsContainer>
        {achievements.slice(0, 9).map(achievements => {
          return (
            <AchievementsContent key={achievements.url}>
              <div className="title">
                <h2>{achievements.name.en}</h2>
                <h1>{achievements.year.en}</h1>
                <h3>
                  <Link href={achievements.url} target="_blank">
                    View the Certificate
                  </Link>
                </h3>
                <span>Prize: {achievements.prize.en}</span>
              </div>
              ...
              <div className="image">
                ...
                >
                  {achievements.image.map((image: any, index: Key | null | undefined) => (
                    <div key={index}>
                      <Image src={image} alt={achievements.name.en} width={300} height={300} loading="lazy" />
                    </div>
                  ))}
                </Carousel>
              </div>
              ...
              <div className="description">
                <span>{achievements.description.en}</span>
              </div>              
            </AchievementsContent>
          );
        })}
